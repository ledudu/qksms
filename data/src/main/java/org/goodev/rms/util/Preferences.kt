...
class Preferences @Inject constructor(private val rxPrefs: RxSharedPreferences) {
    ...
    // 是否把接收和发送的消息插入到系统短信 Provider 中，锤子手机插入Provider 会导致短信出现两次
    val insertToProvider = rxPrefs.getBoolean("insertToProvider", true)
    val mmsSize = rxPrefs.getInteger("mmsSize", 300)
    val logging = rxPrefs.getBoolean("logging", false)
    val oneZeroSix = rxPrefs.getBoolean("oneZeroSix", false)
    val oneZeroSixSynced = rxPrefs.getBoolean("oneZeroSixSynced", false)
