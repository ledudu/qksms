 ...
 */
package org.goodev.rms.feature.themepicker
...
import org.goodev.rms.common.util.extensions.setBackgroundTint
import org.goodev.rms.common.util.extensions.setTint
import org.goodev.rms.common.util.extensions.setVisible
    ...
    override fun onBindViewHolder(holder: QkViewHolder, position: Int) {
        ...
        (palette.subList(0, 5) + palette.subList(5, 10).reversed())
                .mapIndexed { index, color ->
                    val itemView: View = LayoutInflater.from(context).inflate(R.layout.theme_list_item, view.palette, false)

                    // Send clicks to the selected subject
                    itemView.setOnClickListener { colorSelected.onNext(color) }

                    // Apply the color to the view
                    itemView.theme.setBackgroundTint(color)

                    // Control the check visibility and tint
                    itemView.check.setVisible(color == selectedColor)
                    itemView.check.setTint(iconTint)

                    // Update the size so that the spacing is perfectly even
                    itemView.layoutParams = (itemView.layoutParams as FlexboxLayout.LayoutParams).apply {
                        height = size
                        width = size
